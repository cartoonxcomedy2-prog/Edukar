...
function renderDetail() {
    ...
    // ====== INFO STRIP (only show cards with actual data) ======
    ...
    var allSections = [
        { id: 'programs', icon: 'fa-graduation-cap', title: 'Available Programs (' + (sch.programs || []).length + ')', render: renderPrograms, hasData: (sch.programs || []).length > 0 },
        { id: 'coverage', icon: 'fa-wallet', title: 'Financial Coverage', render: renderCoverage, hasData: Array.isArray(sch.coverage) && sch.coverage.length > 0 && sch.coverage.some(function(c) { return c && String(c).trim(); }) },
        { id: 'eligibility', icon: 'fa-check-circle', title: 'Eligibility Criteria', render: renderEligibility, hasData: !!(sch.eligibility && ((sch.eligibility.minPercentage != null && String(sch.eligibility.minPercentage).trim()) || (sch.eligibility.description && sch.eligibility.description.trim()))) },
        { id: 'provider', icon: 'fa-building', title: 'Provider Info', render: renderProvider, hasData: !!(sch.provider && String(sch.provider).trim()) },
        { id: 'contact', icon: 'fa-phone-alt', title: 'Contact Info', render: renderContacts, hasData: (Array.isArray(sch.contactInfo) && sch.contactInfo.length > 0 && sch.contactInfo.some(function(c) { return (c.email && c.email.trim()) || (c.phone && c.phone.trim()); })) || !!(sch.contact && String(sch.contact).trim()) }
    ];
