...
const { sendEmail, getApplicationSubmittedHTML, getStatusUpdateHTML } = require('../utils/emailUtils');
const {
    deleteUploadedFile,
    downloadStoredFile,
    ...
    try {
        ...
        if (type === 'University') {
            targetOpportunity = await University.findById(targetId).select(
                /'_id name isActive deadline eligibility programs city country thumbnail logo image'/
            );
            ...
            targetOpportunity = await Scholarship.findById(targetId).select(
                /'_id title isActive deadline eligibility programs city country thumbnail logo image'/
            );
            if (!targetOpportunity) {
        ...
        if (existing) {
            ...
            try {
                const loc = [targetOpportunity.city, targetOpportunity.country].filter(Boolean).join(', ');
                const thumb = targetOpportunity.thumbnail || targetOpportunity.logo || targetOpportunity.image;
                const eHtml = getApplicationSubmittedHTML(userForSnapshot_re.name, type, targetOpportunity.name || targetOpportunity.title, loc, thumb);
                sendEmail(userForSnapshot_re.email, '✅ Application Submitted Successfully! - EduKar', eHtml);
        ...
        try {
            const loc = [targetOpportunity.city, targetOpportunity.country].filter(Boolean).join(', ');
            const thumb = targetOpportunity.thumbnail || targetOpportunity.logo || targetOpportunity.image;
            const eHtml = getApplicationSubmittedHTML(userForSnapshot.name, type, targetOpportunity.name || targetOpportunity.title, loc, thumb);
            sendEmail(userForSnapshot.email, '✅ Application Submitted Successfully! - EduKar', eHtml);
        } catch(ee) { console.error('Email error:', ee); }
    ...
    try {
        ...
        if (req.files && typeof req.files === 'object' && !Array.isArray(req.files)) {
            ...
            if (req.files.admitCard?.[0]) {
                ...
                ], { originalName: req.files.admitCard[0].originalname });
            ...
            if (req.files.offerLetter?.[0]) {
                ...
                ], { originalName: req.files.offerLetter[0].originalname });
...
const downloadApplicationBundle = async (req, res) => {
    try {
        ...
        // Only education documents go into the ZIP
        ...
        const downloadName = req.query.downloadName || `${userLabel}-education-documents.zip`;
        ...
        try {
            ...
            console.error('[ZIP BUNDLE] Failed to generate summary PDF:', pdfErr.message);
        }
        ...
        for (const doc of docs) {
            try {
                ...
                if (!fileData || !fileData.buffer) continue;
                ...
                console.error(`[ZIP BUNDLE] Failed to add file:`, fileErr.message);
            }
        }

        await archive.finalize();
    } catch (error) {
        console.error('[ZIP BUNDLE ERROR]', error.message);
        if (!res.headersSent) {
