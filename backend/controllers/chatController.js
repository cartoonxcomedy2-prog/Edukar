...
const buildOutOfScopeReply = (role = 'user') => {
    if (role === 'admin') {
        return 'Main sirf EduKar portal ke data aur workflows par answer deta hoon. Is se bahar queries ka jawab nahi deta.';
    }
    if (role === 'university' || role === 'scholarship') {
        return 'Main sirf aapke EduKar admin scope ke data par madad karta hoon. Irrelevant queries ka jawab nahi de sakta.';
    }
    return 'Main sirf EduKar app (universities, scholarships, documents, applications) se related sawalon ka jawab deta hoon.';
};
...
const buildFallbackReply = ({ intent, scope, suggestions, diagnostics }) => {
    ...
    return 'EduKar AI is ready. Ask about eligibility, missing documents, or why an application was not selected.';
};
...
const buildSystemPrompt = (scopeRole) => {
    const roleLine =
        scopeRole === 'admin'
            ? 'You are EduKar Super Admin AI analyst.'
            : scopeRole === 'university'
            ? 'You are EduKar University Sub-Admin AI assistant.'
            : scopeRole === 'scholarship'
            ? 'You are EduKar Scholarship Sub-Admin AI assistant.'
            : 'You are EduKar student counselor AI assistant.';
    ...
    return `${roleLine}
Rules:
- You must ONLY answer questions related to EduKar, universities, scholarships, education, applications, merit, and greetings.
- If a user asks a general knowledge question (e.g., about the world, weather, politics), politely decline and state you only help with EduKar.
