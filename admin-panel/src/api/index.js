...
const DEFAULT_API_BASE = 'https://supadmin.edukar.info/api';
...
const redirectToLogin = () => {
  ...
  if (usesHashRouting) {
    // Extract current path from hash e.g. '#/users?foo=bar' -> '/users?foo=bar'
    const currentHashPath = hash.slice(1) || '/'; // Remove the '#'
    if (currentHashPath.startsWith('/login')) return; // Already on login
    const encodedNext = encodeURIComponent(currentHashPath);
