  ...
  useEffect(() => {
    ...
    const verifySession = async () => {
      ...
      try {
        ...
        // Network/cold-start failure — use fallback stored session.
        // BUT validate role first — never auto-login with 'user' role or empty/corrupt data.
        const fallbackAdmin = parseStoredAdmin();
        const fallbackRole = String(fallbackAdmin?.role || '').trim();
        const VALID_ADMIN_ROLES = ['admin', 'university', 'scholarship'];
        if (!fallbackRole || !VALID_ADMIN_ROLES.includes(fallbackRole)) {
          clearSession();
          setIsAuthenticated(false);
          setAdmin({});
          return;
